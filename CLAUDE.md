# CLAUDE.md — the schema of this wiki

This file tells an agent how to read, extend and check this repository. It is the
contract; `AGENTS.md` is the same contract without Claude-specific names. Read it before
doing anything else here. There are no lints, builders, hooks or CI: every rule below is
a convention you follow, and check, by hand before you commit.
...
[templates/pages/frontmatter.schema.yaml](templates/pages/frontmatter.schema.yaml) is
the written-out reference for these fields. Nothing validates against it automatically.
...
Commit messages are `<type>: <description>` with type one of feat, fix, refactor, docs,
...
| `.claude/` | the repo-local kit: settings, house rules, skills |
| `raw/` | the immutable raw layer |

A new community branch is a copy of `templates/community/`; see
